...
<element id="c6def221-3126-4029-bf1d-87abb5341e41" data-section-id="c6def221-3126-4029-bf1d-87abb5341e41">
    ...
    <htmlCode>
        <div id="root" class="min-h-screen bg-neutral-900">
            <div x-data="{ isOpen: true, showAddExpense: true }" class="min-h-screen">
                <!-- Main Content -->
                <div class="lg:pl-64">
						    ...
						    <div class="flex items-center justify-center min-h-screen px-4">
						        ...
						        <div class="relative bg-neutral-800 rounded-lg max-w-3xl w-full p-6">
						            <div class="flex justify-between items-center mb-4">
						                <h3 class="text-xl font-bold text-white">Update Expense</h3>
						            </div>
						            <form class="space-y-4" action="addnewexpense" method="post">
						                        ...
						                        <select name="category_no"
						                            ...
						                            <c:forEach items="${categories}" var="c" varStatus="status">
						                                <option value="${c.categoryId}"
						                                	<c:if test="${c.categoryId == expense.category.categoryId}">selected</c:if>>
						                                	${c.title}
						                                </option>
						                            </c:forEach>
						                        </select>
						                    ...
						                    <div>
						                        ...
						                        <select name="subcategory_no"
						                            ...
						                            <c:forEach items="${subcategories}" var="sc" varStatus="status">
						                                <option value="${sc.subCatId}"
						                                	<c:if test="${sc.subCatId == expense.subcategory.subCatId}">selected</c:if>>
						                                	${sc.title}
						                                </option>
						                            </c:forEach>
						                        </select>
						                        ...
						                        <select name="vendor_no"
						                            ...
						                            <c:forEach items="${vendors}" var="v" varStatus="status">
						                                <option value="${v.vendorId}"
						                                	<c:if test="${v.vendorId == expense.vendor.vendorId}">selected</c:if>>
						                                	${v.title}
						                                </option>
						                            </c:forEach>
						                        </select>
						                    ...
						                    <div>
						                        ...
						                        <select name="account_no"
						                            ...
						                            <c:forEach items="${accounts}" var="a" varStatus="status">
						                                <option value="${a.id}"
						                                	<c:if test="${a.id == expense.account.id}">selected</c:if>>
						                                	${a.title}
						                                </option>
						                            </c:forEach>
						                        </select>
						                ...
						                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						                    <div>
						                        ...
						                        <select name="status" value="${expense.status}"
						                            class="w-full px-3 py-2 bg-neutral-700 border border-neutral-600 rounded-lg text-white focus:outline-none focus:border-blue-500">
						                            <option value="Done" ${expense.status == 'Done' ? 'selected' : ''}>Done</option>
						                            <option value="Not Done" ${expense.status == 'Not Done' ? 'selected' : ''}>Not Done</option>
						                        </select>
						                    </div>
						                    <div>
						                        <label class="block text-sm font-medium text-neutral-300 mb-1">Transaction Date</label>
						                        <input type="date" name="transactionDate" value="${expense.transactionDate}"
						                            class="w-full px-3 py-2 bg-neutral-700 border border-neutral-600 rounded-lg text-white focus:outline-none focus:border-blue-500">
						                    </div>
						                </div>
						                <!-- Description -->
